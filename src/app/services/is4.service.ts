...
export class IS4Service {
    ...
    accessGetResources(): Observable<BaseEntityResponse<Resource[]>> {
        const url = AppConfigService.getUrlCmsApi(ConfigSetting.IS4_ACCESS_GET_RESOURCES);
    ...
    accessCheckPermission(resourceKey: string, action: string): Observable<BaseResponse> {
        const url = AppConfigService.getUrlCmsApi(ConfigSetting.IS4_ACCESS_CHECK_PERMISSION);
